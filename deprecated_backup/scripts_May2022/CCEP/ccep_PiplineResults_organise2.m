...
cohort_result_folder = fullfile(home_dir, 'Dropbox/Stanford_Matters/data/THAL/COHORT');
...
tic
    ...
    for sn = loopin
        ...
        subjVar.elinfo = subjVar.elinfo(~strcmp(subjVar.elinfo.JP_label, 'empty'),:);
        ...
        parfor i = 1:size(idMat,1)
            ...
            index = find(strcmp(HotT.sbj_name, sbj_id) &...
    ...
    excludeIdx = ((strcmp(metaT.JP_label_in1, 'EXCLUDE') & strcmp(metaT.JP_label_in2, 'EXCLUDE')) | ...
        (strcmp(metaT.JP_label_out1, 'EXCLUDE') & strcmp(metaT.JP_label_out2, 'EXCLUDE'))) | ...
        cellfun(@strcmp, metaT.stim_chan, metaT.record_chan);
    ...
    JP_label_in(strcmp(metaT.JP_label_in1, 'EXCLUDE')) = ...
        metaT.JP_label_in2(strcmp(metaT.JP_label_in1, 'EXCLUDE'));
    JP_label_out = metaT.JP_label_out1;
    JP_label_out(strcmp(metaT.JP_label_out1, 'EXCLUDE')) = ...
        metaT.JP_label_out2(strcmp(metaT.JP_label_out1, 'EXCLUDE'));
    % 2. decide what is cross border pair, and write as a separate column
    % stim
    noExclusion = ~(strcmp(metaT.JP_label_out1, 'EXCLUDE') | strcmp(metaT.JP_label_out2, 'EXCLUDE'));
    sCrossBorder = zeros(size(metaT,1),1);
    sCrossBorder(noExclusion & ~cellfun(@isequal, metaT.JP_label_out1, metaT.JP_label_out2)) = 1;
    % record
    noExclusion = ~(strcmp(metaT.JP_label_in1, 'EXCLUDE') | strcmp(metaT.JP_label_in2, 'EXCLUDE'));
...
%{
        JPlabel_stim = subT.JP_label_out(intersect(StimIdx, find(~strcmp(subT.JP_label_out, 'EXCLUDE'))));
        JPlabel_record = subT.JP_label_in(intersect(RecIdx, find(~strcmp(subT.JP_label_in, 'EXCLUDE'))));
...
%}
            figname = sprintf('Total_RecElecCoverage_Recording_%s_%s_surf2.jpg', per, hem);
...
[activated_redo, act_loc] = redetect_activation(metaT, CCEP_meanTr); %may take a few minutes
