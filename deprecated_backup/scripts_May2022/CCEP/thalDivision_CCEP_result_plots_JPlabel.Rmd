...
```{r total coverage, fig.width=9, fig.height=4}
intable <- table(dTHinUnique$JP_label_in)
outtable <- table(dTHoutUnique$JP_label_out)
...
#Savage('~/Dropbox/Stanford_Matters/data/THAL/CCEP/Plots/explore2/Elec_coverage/Barplot_elec_coverage_thoughTH.png')
```
...
pktimes <- data.matrix(d0[,grepl('pks_time_', names(d))])
```

```{r k-means clustering, fig.width=5, fig.height=4}
...
t1 <- read.csv('/media/lvdian/Dropbox/Stanford_Matters/data/THAL/COHORT/Plots/Elec_coverage/antTH_Count_breakDown_table_rec_channels_outflow.csv')
t2 <- read.csv('/media/lvdian/Dropbox/Stanford_Matters/data/THAL/COHORT/Plots/Elec_coverage/antTH_Count_breakDown_table_stim_channels_inflow.csv')
t3 <- read.csv('/media/lvdian/Dropbox/Stanford_Matters/data/THAL/COHORT/Plots/Elec_coverage/pstTH_Count_breakDown_table_rec_channels_outflow.csv')
t4 <- read.csv('/media/lvdian/Dropbox/Stanford_Matters/data/THAL/COHORT/Plots/Elec_coverage/pstTH_Count_breakDown_table_stim_channels_inflow.csv')
n <- 0
...
area1<- Area1$JP_label[Area1$nSubs >= 2]

for (jplabel in area1){
path = 'outflow'
print('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>')
fprintf('Statistics for CECS score in the %s pathway CCEP to %s between antTH and pstTH.\n', path, jplabel)
...
area2<- Area2$JP_label[Area2$nSubs >= 2]
for (jplabel in area2){
path = 'inflow'
print('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>')
fprintf('Statistics for CECS score in the %s pathway CCEP from %s between antTH and pstTH.\n', path, jplabel)
