...
p<-ggplot(df_thal2,aes(x=factor(JP_label, levels = anatomy_order), y=Percentage_activation, fill=ROI, alpha = pathway))+
  ...
  scale_fill_manual(values = colors) +
  facet_wrap(~ROI, ncol = 1, scales="free_y") +
 labs(title=sprintf('Thalamus CCEP activation profile - later peaks: %s', txt3), x='Brain Area', y='Percentage', fill = "pathway")+
 theme_classic2() +
    theme(axis.text.x = element_text(angle = 30, hjust=0.8, vjust=1),
        text = element_text(size = 11))
p
...
figname<-sprintf('%s/Barplots_latePeakAct_PATH_ROI_interaction.png', figpath)
png(figname,
    width     = 10,
    height    = 6,
    units     = "in",
...
din_roi<- d0 %>% filter(JP_label_in != roi & JP_label_out == roi) %>% 
  ...
  select(subject, aSubID, ROI, pathway, elec_ID, JP_label,osc_speed, min_pk_time, Net1, Net2, activated, activated_default, activated_SimRule, CECS, CECS_activation, pk1, pk2, pk3) 
