...
addpath(fullfile(home_dir, 'Dropbox/scripts/Stanford/ThalamocoricalLoop-project/utils'))
...
tic
    ...
    % reorder the subtable so that aSubID is in the front (1st = subject original ID)
     ...
     % load previous metaT
    clear metaT0
    load(fullfile(cohort_result_folder, 'metaT.mat'));
    metaT2 = [metaT; metaT_new];

    metaT2.aSubID(strcmp(metaT2.aSubID, 'S01_205')) = {'S27_205'};

    %% save metaT
   writetable(metaT2, fullfile(cohort_result_folder, 'table_CCEPnewpipOutput_wholebrain_anatomical_info2.csv'));
   save(fullfile(cohort_result_folder, 'metaT2.mat'), 'metaT2')
   metaT = metaT2;
   clear metaT2
   toc
end
...
if explore_metaT == 1
    ...
    metaT = readtable(fullfile(cohort_result_folder, 'table_CCEPnewpipOutput_wholebrain_anatomical_info2.csv'));
...
if flatten_CCEP == 1
    ...
    metaT2 = metaT;
    metaT = metaT2(ismember(metaT.subject,subject_names), :);
    ...
    %% save flat CCEP
    d1 = load(fullfile(data_result_folder, 'CCEP_all_flat_meanTr.mat'));
    d2 = load(fullfile(data_result_folder, 'CCEP_all_flat_medianTr.mat'));
    d1.CCEP_flat = [d1.CCEP_flat; CCEP_flat];
    d2.CCEP_flat2 = [d2.CCEP_flat2; CCEP_flat2];
...
writetable(metaT, fullfile(cohort_result_folder, 'table_CCEPnewpipOutput_wholebrain_anatomical_info_activationRedone2.csv'));
